...
# Generated shifted kernel for network using non-shifted spike trains
...
def ker_gen(a, tau, reso_set=1, len_ker=401):
    ...
    half_pt = int((len_ker - 1)/2)
    
    x1 = np.linspace(0, half_pt, half_pt+1)
    
    ker_pre_post = np.zeros([len_ker, 1])
    ker_post_pre = np.zeros([len_ker, 1])
    ker_post_post = np.zeros([len_ker, 1])

    ker_pre_post[:half_pt+1] = a[0] * exp_ker(x1, tau_pre_post, half_pt).reshape([half_pt+1, 1])
    ker_post_pre[:half_pt+1] = a[2] * exp_ker(x1, tau_post_pre, half_pt).reshape([half_pt+1, 1])

    if a[0] > 1e-10:
        ker_post_post[:half_pt+1] = a[3]/a[0] * exp_ker(x1, tau_post_post, half_pt).reshape([half_pt+1, 1])
    else:
        ker_post_post[:half_pt+1] = a[3] * exp_ker(x1, tau_post_post, half_pt).reshape([half_pt+1, 1])
